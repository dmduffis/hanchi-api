/**
 * Upsert Miami and Orlando enclaves without wiping existing data.
 * Then Yelp-sync each (~2–3 calls / enclave).
 *
 * Usage: npx tsx scripts/add-florida-communities.ts
 *    or: npm run communities:add-florida
 */
...
import { FLORIDA_COMMUNITIES as COMMUNITIES } from "./data/florida-communities";
...
async function main() {
  console.log(`Upserting ${COMMUNITIES.length} Miami / Orlando enclaves…`);
