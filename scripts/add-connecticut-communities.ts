/**
 * Upsert Connecticut enclaves without wiping existing data.
 * Then Yelp-sync each (~2–3 calls / enclave).
 *
 * Usage: npx tsx scripts/add-connecticut-communities.ts
 *    or: npm run communities:add-ct
 */
...
async function main() {
  console.log(`Upserting ${COMMUNITIES.length} Connecticut enclaves…`);
